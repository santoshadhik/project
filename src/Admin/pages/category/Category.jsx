...
const Category = () => {
  ...
  const { data, mutate, error } = useSWR(APIURL + "category", fetcher);
  ...
  const deleteCategory = async (id) => {
    const category = await fetch(`${APIURL}category/${id}`, {
      method: "delete",
      headers: {
          ...
          <table className="w-full text-center shadow-md">
            <thead className="bg-slate-200">
              <tr className="border border-gray-300">
                <th className="py-2 border border-gray-300 px-2">Order</th>
